...
class CrossModalAttention(nn.Module):
    def __init__(self, dim=64):
        ...
        self.lstm = nn.LSTM(input_size=1152*4, hidden_size=512, num_layers=2, batch_first=True)
        self.fc1 = nn.Linear(512, 256)
        self.fc2 = nn.Linear(256, 64)
        self.fcbio1= nn.Linear(16, 64)
        self.dropout1 = nn.Dropout(p=0.25)

    def forward(self, eeg, video,bio):
        video= video.view(video.size(0), video.size(1),video.size(2)* video.size(3))
        video_f, (_, _) = self.lstm(video)
        video_f = video_f[:, -1, :]
        video_f=self.fc1(video_f)
        video_f=self.dropout1(video_f)
        video_f=self.fc2(video_f) # [B, T*C, D]

        eeg=eeg.unsqueeze(1)  # [B, 1, D]
        video_f= video_f.unsqueeze(1)  # [B, 1, D]
        bio=self.fcbio1(bio)
        bio_f=bio.unsqueeze(1)  # [B, 1, D]

        attn_output, _ = self.mha(eeg,video_f,video_f)
        attn_output_r,_= self.mha(video_f,eeg,eeg)  # [B, 1, D]
        attn_output= attn_output.squeeze(1)  # [B, D]
        attn_output_r=attn_output_r.squeeze(1)  # [B, D]
        eeg_video= torch.cat([attn_output_r, attn_output], dim=1)  # [B, D*2]

        attn_output_bio, _ = self.mha(bio_f, video_f, video_f)
        attn_output_r_bio, _ = self.mha(video_f, bio_f, bio_f)  # [B, 1, D]
        attn_output_bio = attn_output_bio.squeeze(1)  # [B, D]
        attn_output_r_bio = attn_output_r_bio.squeeze(1)  # [B, D]
        bio_video = torch.cat([attn_output_r_bio, attn_output_bio], dim=1)  # [B, D*2]

        attn_output_bio_eeg, _ = self.mha(bio_f, eeg, eeg)
        attn_output_r_bio_eeg, _ = self.mha(eeg, bio_f, bio_f)  # [B, 1, D]
        attn_output_bio_eeg = attn_output_bio_eeg.squeeze(1)  # [B, D]
        attn_output_r_bio_eeg = attn_output_r_bio_eeg.squeeze(1)  # [B, D]
        bio_eeg = torch.cat([attn_output_r_bio_eeg, attn_output_bio_eeg], dim=1)  # [B, D*2]

        out=torch.cat([eeg_video,bio_video,bio_eeg], dim=1)  # [B, D*2]
...
class Encoder(nn.Module):
    def __init__(self, in_planes=[4, 32], layers=2, hidden_1=256, hidden_2=64, class_nums=2):
        ...
        self.fc2 = nn.Linear(hidden_2, 64)
        self.dropout1 = nn.Dropout(p=0.5)
        self.dropout2 = nn.Dropout(p=0.5)
        self.fc0=nn.Linear(256,128)
        self.norm=nn.BatchNorm1d(128)
        self.transformer = nn.TransformerDecoderLayer(d_model=1152, nhead=8, dim_feedforward=512, dropout=0.1)
...
class Domain_adaption_model(nn.Module):
    def __init__(self, in_planes=[4, 32], layers=2, hidden_1=256, hidden_2=64, num_of_class=2, device='cuda:0',
                 source_num=1240):
        super(Domain_adaption_model, self).__init__()
        self.encoder = Encoder(in_planes=in_planes, layers=layers, hidden_1=hidden_1, hidden_2=hidden_2,
                               class_nums=num_of_class)
        self.cls_classifier = ClassClassifier(hidden_2=hidden_2, num_cls=num_of_class)
        self.cross_modal_attn = CrossModalAttention(dim=hidden_2)
    ...
    def forward(self, source, target, source_video,target_video,source_bio,target_bio,source_label, source_index):
        ...
        source_f = self.cross_modal_attn(source_f, source_video,source_bio)

        target_f = self.cross_modal_attn(target_f, target_video,target_bio)
        ...
        return source_predict, source_f, target_predict, target_f, [self.src_adj, self.src_sa, self.src_ca], [
            self.tar_adj, self.tar_sa, self.tar_ca], target_label
    ...
    def get_target_labels(self, feature_source_f, source_label_feature, source_index, feature_target_f):
        ...
        entropy = self.Entropy(target_predict)
        if entropy.mean() < 0.1:  # Threshold to detect low entropy
            target_predict = target_predict + 0.1 * torch.ones_like(target_predict) / self.num_of_class  # Soften labels
            target_predict = torch.nn.functional.softmax(target_predict, dim=1)

        return target_predict

    def get_init_banks(self, source, source_index,source_video_f,source_bio):
        self.eval()
        source_eeg_f, source_att = self.encoder(source)
        source_f = self.cross_modal_attn(source_eeg_f, source_video_f,source_bio)
        source_predict = self.cls_classifier(source_f)
    ...
    def target_predict(self, feature_target,video_target,bio_target):
        self.eval()
        target_f, _ = self.encoder(feature_target)
        target_f = self.cross_modal_attn(target_f, video_target,bio_target)
        target_predict = self.cls_classifier(target_f)
    ...
    def get_weight(self, score_near):
        ...
        g = 1 - entropy
        score_near_weight = g / (torch.sum(g, dim=1).view(-1, 1) + epsilon)
